...
void TEST_MiddleFielderPlayer() {
	cout << "+----------------------------+" << endl << "| MIDDLE FIELDER PLAYER TEST |" << endl << "+----------------------------+" << endl; 
	MiddleFielderPlayer* middleFielder = new MiddleFielderPlayer("Gareth Bale", 25, 15, 0.2);
	middleFielder->Move(Player::Direction::EAST); 
	middleFielder->Move(Player::Direction::NORTH_WEST);
	middleFielder->Shot();
	...
	forward->setGoalCount(42); cout << "Payment for this year : " << forward->CalculatePaymentForThisYear() << " million EUR" << endl;
}
int main() {
	TEST_GoalKeeper();
